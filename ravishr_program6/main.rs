...
//  main.c
//  assignment 6
//  Inline comments and citations. Majority of program was provided. Just added/changed
//  portions that explicitly gave us permission to modify/add code.
//

use std::env; // to get arugments passed to the program
use std::thread; // to use thread
use thread::JoinHandle; // to be able to create a vector of threads
...
fn main() {
    ...
    // Change the following code to create 2 threads that run concurrently and each of which
    // uses map_data() function to process one of the two partitions
    let xs1 = xs.clone();  // need to clone so that after 'move' it can still access xs
    let t1 = thread::spawn(move|| map_data(&xs1[0]));

    let xs2 = xs.clone();  // need to clone so that after 'move' it can still access xs
    ...
    let pd = partition_data(&num_partitions, &v);

    // 2. Calls print_partition_info to print info on the partitions that have been created
    print_partition_info(&pd);

    // 3. Creates one thread per partition and uses each thread to concurrently process
    // one partition
    let mut handles : Vec<JoinHandle<usize>> = Vec::new();  // Create vector to hold threads
    for i in 0..num_partitions {
        let pd_clone = pd.clone();  // Again, clone to avoid error with accessing pd
        handles.push(thread::spawn(move|| map_data(&pd_clone[i])));
    }
    // 4. Collects the intermediate sums from all the threads
    let mut intermediate_sums_pd : Vec<usize> = Vec::new();
    for handle in handles {
        let _res = handle.join().unwrap();  // join threads
        intermediate_sums_pd.push(_res);
    }
    ...
    let sum_pd = reduce_data(&intermediate_sums_pd);
    ...
    */
    fn partition_data(num_partitions: &usize, v: &Vec<usize>) -> Vec<Vec<usize>> {
        //println!("In partition data method");
        let partition_size = v.len() / num_partitions;
        let mut _remainder = v.len() % num_partitions;  // store remainder, if any
        // Create a vector that will contain vectors of integers
        let mut pd: Vec<Vec<usize>> = Vec::new();

        // will help loop through entire vector of integers and hold place in vector
        let mut start_range = 0;
        let mut end_range = partition_size;

        while start_range != v.len() {
            //println!("Start Range is {}", start_range);
            //println!("End Range is {}", end_range);
            ...
            // Add x to the vector that will be returned by this function
            pd.push(x);
            start_range = end_range;  // set start_range to the index held by end_range
            // If no remainder, then check to see if we already looped through entire vector
            if _remainder == 0 {
                if end_range == v.len() {
                    break;
                }
            }
            // If there is remainder, check to see when all partitions have the max
            // number of elements (where all of them are the same size).
            else {
                if end_range == v.len() - _remainder {
                    break;
                }
            }
            end_range = end_range + partition_size;
        }
        // If there was remainder, go back and add the remaining integers in vector to some
        // partitions
        if _remainder != 0 {
            let mut _index = 0;
            for i in end_range..v.len() {
                pd[_index].push(v[i]);
                _index = _index + 1;
            }
        }
